...
public class JavaOCLLogicCircuitEmulator {
	...
	public static void main(String[] arg) {
		...
		if (arg.length>1) {
			...
			emulator.run(filememory, fileboot, filecart, filememout, filecartout, filedispout, cycles, cores);
		} else {
			System.out.println("arguments expected: mem.bin boot.bin cart.bin mem.out cart.out disp.out [cores] [cycles]");
		}
		System.out.println("exit.");
	}
	
	public void run(String filememory, String fileboot, String filecart, String filememout, String filecartout, String filedispout, long cycles, int cores) {
		...
		public void loadmemory(long[] memory) {
			if (memory!=null) {
				for (int i=0;(i<memory.length)&&(i<memoryamount);i++) {
					oldmemoryram[i] = memory[i];
					newmemoryram[i] = memory[i];
				}
			}
		}
	...
	public class RiscCore {
		...
		private long timer = systemnanotime;
		...
		private long corerun = 0xffffffffffffffffL;
		...
		public RiscCore(int corenumi) {
			...
			threadbase = 0xCAL - 0x30L * corenumi;
			memorybase = 0x00L;
			registermask = 0L;
			memorymask = 0L;
		}
		...
		public void processinstruction() {
			if (firsttime) {
				...
				riscchip.newmemoryram[(int)threadbase+5] = corerun;
				...
				riscchip.oldmemoryram[(int)threadbase+5] = corerun;
				counter = 10;
				threadcycleind = 4;
				firsttime = false;
			}
			...
			threadbase = riscchip.oldmemoryram[(int)threadroot+7];
			...
			registermask = riscchip.oldmemoryram[(int)threadbase+3];
			memorymask = riscchip.oldmemoryram[(int)threadbase+4];
			corerun = riscchip.oldmemoryram[(int)threadbase+5];
			registerbase = threadbase + 6;
			...
			System.out.println("core: "+String.format("%04x", corenum)+", thread("+threadcycleind+"): "+String.format("%04x", threadbase)+", cycle: "+String.format("%08x", cyclenum)+", programcounter: "+String.format("%016x", programcounter)+", instructionstate: "+String.format("%016x", instructionstate)+", instructionstep: "+String.format("%08x", instructionstep)+
					...
					", rC:"+String.format("%016x", riscchip.oldmemoryram[(int)registerbase+0xC])+", rD:"+String.format("%016x", riscchip.oldmemoryram[(int)registerbase+0xD])+", rE:"+String.format("%016x", riscchip.oldmemoryram[(int)registerbase+0xE])+", rF:"+String.format("%016x", riscchip.oldmemoryram[(int)registerbase+0xF])
					);
			instbytes.clear();
			...
			int registerbaseregZ = (int)registerbase+(int)(regZ&(~registermask));
			if (registerbaseregX>=RiscChip.memoryamount) {registerbaseregX=0;}
			if (registerbaseregY>=RiscChip.memoryamount) {registerbaseregY=0;}
			if (registerbaseregZ>=RiscChip.memoryamount) {registerbaseregZ=0;}
			...
			switch(insT) {
				...
				case 0x20: if (true) {
					programcounter = oldregistersregX;
					riscchip.newmemoryram[(int)threadbase] = programcounter;
				} break;
				case 0x30: if (true) {
					long jumpflag = oldregistersregY;
					if (jumpflag!=0) {
						programcounter = oldregistersregX;
					} else {
						if (corerun!=0) { programcounter++; }
				...
				default: if (true) {
					...
					for (int i=0;i<8;i++) {
						if (vecnbits.get(i)) {
							long oldregistersregXpi = riscchip.oldmemoryram[registerbaseregX+i];
							long oldregistersregYpi = riscchip.oldmemoryram[registerbaseregY+i];
							long oldregistersregZpi = riscchip.oldmemoryram[registerbaseregZ+i];
							long newregistersregXpi = oldregistersregXpi;
					
							switch(insT) {
								case 0x40: if (true) {
									newregistersregXpi = regYZ;
								} break;
								case 0x50: if (true) {
									newregistersregXpi = oldregistersregY;
								} break;
								case 0x60: if (true) {
									longbytes.clear();
									longbytes.putLong(oldregistersregY).rewind();
									...
									newregistersregXpi = longbytesvalue;
								} break;
								case 0x70: if (true) {
									longbytes.clear();
									longbytes.putLong(oldregistersregY).rewind();
								...
								case 0x80: if (true) {
									...
									longbytes.put(2, bytevalue1).rewind();
									longbytes.put(3, bytevalue1).rewind();
									longbytes.put(4, bytevalue1).rewind();
								...
								case 0x90: if (true) {
									newregistersregXpi = this.counter;
								} break;
								case 0xa0: if (true) {
									newregistersregXpi = this.randoms[i].nextLong();
								} break;
								case 0xb0: if (true) {
									if (oldregistersregYpi==1) {
										newregistersregXpi = riscchip.risccores.length;
									} else if (oldregistersregYpi==2) {
										newregistersregXpi = 65536L;
									} else if (oldregistersregYpi==3) {
										newregistersregXpi = RiscChip.memoryamount;
									} else if (oldregistersregYpi==4) {
										newregistersregXpi = RiscChip.displaywidth;
									} else if (oldregistersregYpi==5) {
										newregistersregXpi = RiscChip.displayheight;
									} else if (oldregistersregYpi==6) {
										newregistersregXpi = riscchip.clockfrequency;
									} else if (oldregistersregYpi==7) {
										newregistersregXpi = RiscChip.coreversion;
									} else {
										newregistersregXpi = corenum;
									}
								} break;
								case 0xc0: if (true) {
								...
								case 0xe0: if (true) {
									...
									if (regybit63 || regybit62) {
										...
										if (regybit63) {
											newregistersregXpi = riscchip.displayram[((int)memaddress)+i];
										} else {
											newregistersregXpi = riscchip.cartram[((int)memaddress)+i];
										}
									} else {
										newregistersregXpi = riscchip.oldmemoryram[memorybaseregY+i];
									}
								} break;
								case 0xf0: if (true) {
									long[] regyaddr = {oldregistersregY};
									...
									if (regybit63 || regybit62) {
										...
										if (regybit63) {
											riscchip.displayram[((int)memaddress)+i] = oldregistersregXpi;
										} else {
											riscchip.cartram[((int)memaddress)+i] = oldregistersregXpi;
										}
									} else {
										riscchip.newmemoryram[memorybaseregY+i] = oldregistersregXpi;
									}
								} break;
								default: if (true) {
	...
	public static long alucompute(int insT, long oldregistersregXpi, long oldregistersregYpi, long oldregistersregZpi) {
		...
		switch(insT) {
			case 0x01: if (true) {
				...
				if (oldregistersregYpi==0) {
					newregistersregXpi = 1;
				}
			} break;
			case 0x11: if (true) {
				newregistersregXpi = 0;
			...
			case 0x02: if (true) {
				newregistersregXpi = 0;
				if (oldregistersregYpi==oldregistersregZpi) {
					newregistersregXpi = 1;
				}
			} break;
			case 0x12: if (true) {
				...
				longbytes.putLong(oldregistersregYpi).rewind();
				longbytes2.clear();
				longbytes2.putLong(oldregistersregZpi).rewind();
			...
			case 0x03: if (true) {
				newregistersregXpi = -oldregistersregYpi;
			} break;
			case 0x13: if (true) {
				newregistersregXpi = 0;
				longbytes.clear();
			...
			case 0x53: if (true) {
				...
				longbytes.putLong(oldregistersregXpi).rewind();
				if (changevalue1!=0) {
					longbytes.putInt(0, intvalue1).rewind();
				}
				if (changevalue2!=0) {
			...
			case 0x24: if (true) {
				newregistersregXpi = oldregistersregYpi | oldregistersregZpi;
			} break;
			case 0x34: if (true) {
				newregistersregXpi = oldregistersregYpi & oldregistersregZpi;
			} break;
			...
			case 0x45: if (true) {
				newregistersregXpi = Long.rotateLeft(oldregistersregYpi, (int)oldregistersregZpi);
			} break;
			case 0x55: if (true) {
				newregistersregXpi = 0;
				longbytes.clear();
			...
			case 0x85: if (true) {
				newregistersregXpi = Long.rotateRight(oldregistersregYpi, (int)oldregistersregZpi);
			} break;
			case 0x95: if (true) {
				newregistersregXpi = 0;
				longbytes.clear();
			...
			case 0xc5: if (true) {
				long[] regyarray = {oldregistersregYpi};
				BitSet regybits = BitSet.valueOf(regyarray);
				newregistersregXpi = regybits.cardinality();
			} break;
			case 0xd5: if (true) {
				newregistersregXpi = 0;
				longbytes.clear();
			...
			case 0x46: if (true) {
				...
				newregistersregXpi = regyhighone;
			} break;
			case 0x56: if (true) {
				newregistersregXpi = 0;
				longbytes.clear();
			...
			case 0x86: if (true) {
				...
				newregistersregXpi = regylowzero;
			} break;
			case 0x96: if (true) {
				newregistersregXpi = 0;
				longbytes.clear();
			...
			case 0xc6: if (true) {
				...
				newregistersregXpi = regyhighzero;
			} break;
			case 0xd6: if (true) {
				newregistersregXpi = 0;
				longbytes.clear();
			...
			case 0x08: if (true) {
				newregistersregXpi = 0;
				BigInteger oldregY = BigInteger.valueOf(oldregistersregYpi);
				BigInteger oldregZ = BigInteger.valueOf(oldregistersregZpi);
			...
			case 0x88: if (true) {
				...
				if (newregXover.compareTo(BigInteger.ZERO)!=0) {
					...
					newregistersregXpi = newregXlong;
				}
			} break;
			case 0x98: if (true) {
				newregistersregXpi = 0;
			...
			case 0x09: if (true) {
				...
				newregistersregXpi = longbytes3.getLong();
			} break;
			case 0x19: if (true) {
				newregistersregXpi = 0;
				longbytes.clear();
			...
			case 0xcb: if (true) {
				...
				longbytes2.putLong(oldregistersregZpi).rewind();
				double longdouble2 = longbytes2.asDoubleBuffer().get();
				double longdouble3 = Math.min(longdouble, longdouble2);
				longbytes3.clear();
				longbytes3.putDouble(longdouble3).rewind();
				newregistersregXpi = longbytes3.getLong();
			} break;
			case 0xdb: if (true) {
				...
				float floatvalue2b = longbytes2.getFloat(4);
				longbytes.putLong(0L).rewind();
				float floatvaluea = Math.min(floatvalue1a, floatvalue2a);
				float floatvalueb = Math.min(floatvalue1b, floatvalue2b);
				longbytes.putFloat(0, floatvaluea).rewind();
				longbytes.putFloat(4, floatvalueb).rewind();
				newregistersregXpi = longbytes.getLong();
			} break;
			case 0xeb: if (true) {
				...
				float floatvaluea = Math.min(floatvalue1a, floatvalue2a);
				float floatvalueb = Math.min(floatvalue1b, floatvalue2b);
				float floatvaluec = Math.min(floatvalue1c, floatvalue2c);
				float floatvalued = Math.min(floatvalue1d, floatvalue2d);
				longbytes.putShort(0, floattohalf(floatvaluea)).rewind();
				longbytes.putShort(2, floattohalf(floatvalueb)).rewind();
				longbytes.putShort(4, floattohalf(floatvaluec)).rewind();
				longbytes.putShort(6, floattohalf(floatvalued)).rewind();
			...
			case 0xfb: if (true) {
				...
				float floatvaluec = Math.min(floatvalue1c, floatvalue2c);
				float floatvalued = Math.min(floatvalue1d, floatvalue2d);
				float floatvaluee = Math.min(floatvalue1e, floatvalue2e);
				float floatvaluef = Math.min(floatvalue1f, floatvalue2f);
				float floatvalueg = Math.min(floatvalue1g, floatvalue2g);
				float floatvalueh = Math.min(floatvalue1h, floatvalue2h);
				longbytes.put(0, floattomini(floatvaluea)).rewind();
				longbytes.put(1, floattomini(floatvalueb)).rewind();
				longbytes.put(2, floattomini(floatvaluec)).rewind();
				longbytes.put(3, floattomini(floatvalued)).rewind();
			...
			case 0xcc: if (true) {
				...
				longbytes2.putLong(oldregistersregZpi).rewind();
				double longdouble2 = longbytes2.asDoubleBuffer().get();
				double longdouble3 = Math.max(longdouble, longdouble2);
				longbytes3.clear();
				longbytes3.putDouble(longdouble3).rewind();
				newregistersregXpi = longbytes3.getLong();
			} break;
			case 0xdc: if (true) {
				...
				float floatvalue2b = longbytes2.getFloat(4);
				longbytes.putLong(0L).rewind();
				float floatvaluea = Math.max(floatvalue1a, floatvalue2a);
				float floatvalueb = Math.max(floatvalue1b, floatvalue2b);
				longbytes.putFloat(0, floatvaluea).rewind();
				longbytes.putFloat(4, floatvalueb).rewind();
				newregistersregXpi = longbytes.getLong();
			} break;
			case 0xec: if (true) {
				...
				float floatvaluea = Math.max(floatvalue1a, floatvalue2a);
				float floatvalueb = Math.max(floatvalue1b, floatvalue2b);
				float floatvaluec = Math.max(floatvalue1c, floatvalue2c);
				float floatvalued = Math.max(floatvalue1d, floatvalue2d);
				longbytes.putShort(0, floattohalf(floatvaluea)).rewind();
				longbytes.putShort(2, floattohalf(floatvalueb)).rewind();
				longbytes.putShort(4, floattohalf(floatvaluec)).rewind();
				longbytes.putShort(6, floattohalf(floatvalued)).rewind();
			...
			case 0xfc: if (true) {
				...
				float floatvaluec = Math.max(floatvalue1c, floatvalue2c);
				float floatvalued = Math.max(floatvalue1d, floatvalue2d);
				float floatvaluee = Math.max(floatvalue1e, floatvalue2e);
				float floatvaluef = Math.max(floatvalue1f, floatvalue2f);
				float floatvalueg = Math.max(floatvalue1g, floatvalue2g);
				float floatvalueh = Math.max(floatvalue1h, floatvalue2h);
				longbytes.put(0, floattomini(floatvaluea)).rewind();
				longbytes.put(1, floattomini(floatvalueb)).rewind();
				longbytes.put(2, floattomini(floatvaluec)).rewind();
				longbytes.put(3, floattomini(floatvalued)).rewind();
			...
			case 0x0d: if (true) {
				...
				longbytes.clear();
				longbytes.putLong(oldregistersregYpi).rewind();
				double doublevalue1a = longbytes.getDouble(0);
				longbytes.putLong(0L).rewind();
				double doublevaluea = Math.log(doublevalue1a);
			...
			case 0x1d: if (true) {
				...
				float floatvalue1b = longbytes.getFloat(4);
				longbytes.putLong(0L).rewind();
				float floatvaluea = (float)(Math.log(floatvalue1a));
				float floatvalueb = (float)(Math.log(floatvalue1b));
				longbytes.putFloat(0, floatvaluea).rewind();
				longbytes.putFloat(4, floatvalueb).rewind();
				newregistersregXpi = longbytes.getLong();
			} break;
			case 0x2d: if (true) {
				...
				float floatvaluea = (float)(Math.log(floatvalue1a));
				float floatvalueb = (float)(Math.log(floatvalue1b));
				float floatvaluec = (float)(Math.log(floatvalue1c));
				float floatvalued = (float)(Math.log(floatvalue1d));
				longbytes.putShort(0, floattohalf(floatvaluea)).rewind();
				longbytes.putShort(2, floattohalf(floatvalueb)).rewind();
				longbytes.putShort(4, floattohalf(floatvaluec)).rewind();
				longbytes.putShort(6, floattohalf(floatvalued)).rewind();
			...
			case 0x3d: if (true) {
				...
				float floatvaluec = (float)(Math.log(floatvalue1c));
				float floatvalued = (float)(Math.log(floatvalue1d));
				float floatvaluee = (float)(Math.log(floatvalue1e));
				float floatvaluef = (float)(Math.log(floatvalue1f));
				float floatvalueg = (float)(Math.log(floatvalue1g));
				float floatvalueh = (float)(Math.log(floatvalue1h));
				longbytes.put(0, floattomini(floatvaluea)).rewind();
				longbytes.put(1, floattomini(floatvalueb)).rewind();
				longbytes.put(2, floattomini(floatvaluec)).rewind();
				longbytes.put(3, floattomini(floatvalued)).rewind();
			...
			case 0x4d: if (true) {
				...
				longbytes.putLong(oldregistersregYpi).rewind();
				double longdouble = longbytes.asDoubleBuffer().get();
				double longdouble3 = Math.exp(longdouble);
				longbytes3.clear();
				longbytes3.putDouble(longdouble3).rewind();
				newregistersregXpi = longbytes3.getLong();
			} break;
			case 0x5d: if (true) {
				...
				float floatvalue1b = longbytes.getFloat(4);
				longbytes.putLong(0L).rewind();
				float floatvaluea = (float)Math.exp(floatvalue1a);
				float floatvalueb = (float)Math.exp(floatvalue1b);
				longbytes.putFloat(0, floatvaluea).rewind();
				longbytes.putFloat(4, floatvalueb).rewind();
				newregistersregXpi = longbytes.getLong();
			} break;
			case 0x6d: if (true) {
				...
				float floatvaluea = (float)Math.exp(floatvalue1a);
				float floatvalueb = (float)Math.exp(floatvalue1b);
				float floatvaluec = (float)Math.exp(floatvalue1c);
				float floatvalued = (float)Math.exp(floatvalue1d);
				longbytes.putShort(0, floattohalf(floatvaluea)).rewind();
				longbytes.putShort(2, floattohalf(floatvalueb)).rewind();
				longbytes.putShort(4, floattohalf(floatvaluec)).rewind();
				longbytes.putShort(6, floattohalf(floatvalued)).rewind();
			...
			case 0x7d: if (true) {
				...
				float floatvaluec = (float)Math.exp(floatvalue1c);
				float floatvalued = (float)Math.exp(floatvalue1d);
				float floatvaluee = (float)Math.exp(floatvalue1e);
				float floatvaluef = (float)Math.exp(floatvalue1f);
				float floatvalueg = (float)Math.exp(floatvalue1g);
				float floatvalueh = (float)Math.exp(floatvalue1h);
				longbytes.put(0, floattomini(floatvaluea)).rewind();
				longbytes.put(1, floattomini(floatvalueb)).rewind();
				longbytes.put(2, floattomini(floatvaluec)).rewind();
				longbytes.put(3, floattomini(floatvalued)).rewind();
			...
			case 0x8d: if (true) {
				...
				longbytes.putLong(oldregistersregYpi).rewind();
				double longdouble = longbytes.asDoubleBuffer().get();
				double longdouble3 = Math.abs(longdouble);
				longbytes3.clear();
				longbytes3.putDouble(longdouble3).rewind();
				newregistersregXpi = longbytes3.getLong();
			} break;
			case 0x9d: if (true) {
				...
				float floatvalue1b = longbytes.getFloat(4);
				longbytes.putLong(0L).rewind();
				float floatvaluea = Math.abs(floatvalue1a);
				float floatvalueb = Math.abs(floatvalue1b);
				longbytes.putFloat(0, floatvaluea).rewind();
				longbytes.putFloat(4, floatvalueb).rewind();
				newregistersregXpi = longbytes.getLong();
			} break;
			case 0xad: if (true) {
				...
				float floatvaluea = Math.abs(floatvalue1a);
				float floatvalueb = Math.abs(floatvalue1b);
				float floatvaluec = Math.abs(floatvalue1c);
				float floatvalued = Math.abs(floatvalue1d);
				longbytes.putShort(0, floattohalf(floatvaluea)).rewind();
				longbytes.putShort(2, floattohalf(floatvalueb)).rewind();
				longbytes.putShort(4, floattohalf(floatvaluec)).rewind();
				longbytes.putShort(6, floattohalf(floatvalued)).rewind();
			...
			case 0xbd: if (true) {
				...
				float floatvaluec = Math.abs(floatvalue1c);
				float floatvalued = Math.abs(floatvalue1d);
				float floatvaluee = Math.abs(floatvalue1e);
				float floatvaluef = Math.abs(floatvalue1f);
				float floatvalueg = Math.abs(floatvalue1g);
				float floatvalueh = Math.abs(floatvalue1h);
				longbytes.put(0, floattomini(floatvaluea)).rewind();
				longbytes.put(1, floattomini(floatvalueb)).rewind();
				longbytes.put(2, floattomini(floatvaluec)).rewind();
				longbytes.put(3, floattomini(floatvalued)).rewind();
			...
			case 0xed: if (true) {
				...
				longbytes.putShort(6, shortvalued).rewind();
				newregistersregXpi = longbytes.getLong();
			} break;
			case 0xfd: if (true) {
				longbytes.clear();
				...
				longbytes.put(3, bytevalued).rewind();
				longbytes.put(4, bytevaluee).rewind();
				longbytes.put(5, bytevaluef).rewind();
				longbytes.put(6, bytevalueg).rewind();
				longbytes.put(7, bytevalueh).rewind();
				newregistersregXpi = longbytes.getLong();
			} break;
			...
			case 0x0e: if (true) {
				...
				longbytes.putLong(oldregistersregYpi).rewind();
				double longdouble = longbytes.asDoubleBuffer().get();
				newregistersregXpi = Math.round(longdouble);
			} break;
			case 0x1e: if (true) {
				newregistersregXpi = 0;
				longbytes.clear();
			...
			case 0x4e: if (true) {
				...
				longbytes.putLong(oldregistersregYpi).rewind();
				double longdouble = longbytes.asDoubleBuffer().get();
				newregistersregXpi = (long)Math.floor(longdouble);
			} break;
			case 0x5e: if (true) {
				newregistersregXpi = 0;
				longbytes.clear();
			...
			case 0x8e: if (true) {
				...
				longbytes.putLong(oldregistersregYpi).rewind();
				double longdouble = longbytes.asDoubleBuffer().get();
				newregistersregXpi = (long)Math.ceil(longdouble);
			} break;
			case 0x9e: if (true) {
				newregistersregXpi = 0;
				longbytes.clear();
			...
			case 0xce: if (true) {
				...
				longbytes.putLong(oldregistersregYpi).rewind();
				double longdouble = longbytes.asDoubleBuffer().get();
				newregistersregXpi = (long)longdouble;
			} break;
			case 0xde: if (true) {
				newregistersregXpi = 0;
				longbytes.clear();
			...
			case 0x0f: if (true) {
				longbytes.clear();
				longbytes.putDouble((double)oldregistersregYpi).rewind();
				newregistersregXpi = longbytes.getLong();
			} break;
			case 0x1f: if (true) {
				newregistersregXpi = 0;
			...
			case 0x4f: if (true) {
				long longvalue1 = oldregistersregYpi;
			...
			case 0x8f: if (true) {
				...
				longbytes.putLong(oldregistersregYpi).rewind();
				int shortvalue1 = longbytes.getShort(0);
				int shortvalue2 = longbytes.getShort(4);
				longbytes.putLong(0L).rewind();
				longbytes.putInt(0, (int)shortvalue1).rewind();
				longbytes.putInt(4, (int)shortvalue2).rewind();
				newregistersregXpi = longbytes.getLong();
			} break;
			case 0x9f: if (true) {
				longbytes.clear();
	...
	public static double gammap1factorial(double x) {
		...
		double lngammaval4 = c1 * xm1;
		double lngammaval5 = c2 * xm2;
		double lngammaval6 = c3 * xm3;
		double lngammaval7 = c4 * xm4;
		double lngammaval8 = c5 * xm5;
